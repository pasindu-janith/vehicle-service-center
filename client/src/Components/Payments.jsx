...
export const Payments = () => {
  ...
  useEffect(() => {
    const loadServiceRecords = async () => {
      try {
        const response = await fetch(`${BASE_URL}/loadServiceRecordPayment`, {
          method: "GET",
          credentials: "include",
        });
        if (response.ok) {
    ...
    const loadCompletedPayments = async () => {
      try {
        ...
        if (response.ok) {
          const data = await response.json();
          console.log("Completed payments fetched successfully:", data);
  ...
  const calculateTotalPending = () => {
    return paymentData.reduce(
      (total, record) => total + parseFloat(record.final_amount || 0),
      0
    );
  };

  const filteredPendingPayments = paymentData.filter(
    (record) =>
        ...
        <div className="col-12">
          <div className="d-flex align-items-center justify-content-between">
            <div className="d-flex align-items-center">
              ...
              <div>
                <h1
                  className="fw-bold main-title mb-1"
          ...
          >
            <div className="card-body text-white text-center p-4">
              ...
              <h3 className="fw-bold mb-1">
                Rs. {calculateTotalPending().toLocaleString()}
              </h3>
                        ...
                        >
                          ...
                          <td className="py-3">
                            <span className="fw-bold text-danger fs-6">
                              Rs.{" "}
                              {parseFloat(record.final_amount).toLocaleString()}
                            </span>
                          </td>
                          <td className="py-3">
                            <button
            ...
            <div className="card-body p-0">
              {completedPayments.length > 0 ? (
                ...
                <div className="text-center py-5">
                  ...
                  <p className="text-muted mb-0">
                    Your completed payments will appear here once you make your
                    first payment.
                  </p>
                </div>
              )}
